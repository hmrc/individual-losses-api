...
class Def1_AmendLossClaimsOrderISpec extends IntegrationBaseSpec {
  ...
  "Calling the Amend Loss Claims Order endpoint" should {
    ...
    "return a 200 status code" when {
      ...
      "any valid request is made for a Tax Year Specific tax year" in new Test {
        // val queryParams = Map("taxYear" -> "23-24")

        override def setupStubs(): StubMapping = {
          AuditStub.audit()
          AuthStub.authorised()
          MtdIdLookupStub.ninoFound(nino)
          // DownstreamStub.onSuccess(DownstreamStub.PUT, downstreamUri, Status.NO_CONTENT, queryParams)
          DownstreamStub.onSuccess(DownstreamStub.PUT, downstreamUri, Status.NO_CONTENT)
        }

        val response: WSResponse = await(request().withQueryStringParameters("taxYear" -> "23-24").put(requestJson()))
  ...
  private trait Test {
    ...
    // val downstreamUri = s"/itsd/income-sources/claims-for-relief/$nino/preferences\\?taxYear=$downstreamTaxYear"
    val downstreamUri = "/itsd/income-sources/claims-for-relief/AA123456A/preferences"
