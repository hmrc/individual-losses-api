...
class Def1_DeleteBFLossControllerItsdISpec extends IntegrationBaseSpec {
  ...
  private trait Test {
    ...
    val downstreamQueryParams: Map[String, String] = Map("taxYear" -> currentTaxYear.asTysDownstream)
    ...
    "return a 204 status code" when {
      ...
      "any valid request is made" in new Test {
        ...
        override def setupStubs(): StubMapping = {
          ...
          DownstreamStub.onSuccess(DownstreamStub.DELETE, downstreamUrl, downstreamQueryParams, NO_CONTENT, JsObject.empty)
        }
      ...
      def serviceErrorTest(desStatus: Int, desCode: String, expectedStatus: Int, expectedBody: MtdError): Unit = {
        s"downstream returns an $desCode error" in new Test {
          ...
          override def setupStubs(): StubMapping = {
            ...
            AuthStub.authorised()
            MtdIdLookupStub.ninoFound(nino)
            DownstreamStub.onError(DownstreamStub.DELETE, downstreamUrl, downstreamQueryParams, desStatus, errorBody(desCode))
          }
