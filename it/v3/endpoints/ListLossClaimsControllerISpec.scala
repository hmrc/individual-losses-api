    ...
    "return a 200 status code" when {
      ...
      "querying for a specific typeOfLoss" in new Test {
        ...
        override def setupStubs(): StubMapping = {
          ...
          AuthStub.authorised()
          MtdIdLookupStub.ninoFound(nino)
          IfsStub.onSuccess(IfsStub.GET, ifsUrl, Map("incomeSourceType" -> "02"), Status.OK, downstreamResponse)
        }
     ...
     """.stripMargin)
        ...
        override def setupStubs(): StubMapping = {
          ...
          AuthStub.authorised()
          MtdIdLookupStub.ninoFound(nino)
          IfsStub.onSuccess(IfsStub.GET, ifsUrl, queryParams = Map("incomeSourceId" -> "XAIS12345678911", "taxYear" -> "2020"), Status.OK, downstreamResponse)
        }
    ...
    "return a 404 status code" when {
      "an empty array (no loss claims exists) is returned from backend" in new Test {
        ...
        override def setupStubs(): StubMapping = {
          ...
          AuthStub.authorised()
          MtdIdLookupStub.ninoFound(nino)
          IfsStub.onSuccess(IfsStub.GET, ifsUrl, Map.empty, Status.OK,
            Json.parse(
              """
                |[
                |
