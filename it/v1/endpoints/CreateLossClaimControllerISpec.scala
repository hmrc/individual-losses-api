  ...
  "Calling the create LossClaim endpoint" should {
    ...
    "return 400 (Bad Request)" when {
      ...
      createLossClaimValidationErrorTest("AA123456A",
        generateLossClaim(Some(selfEmploymentId), typeOfLoss, "20111", "carry-forward") , Status.BAD_REQUEST, TaxYearFormatError.copy(paths = Some(List("/taxYear"))))
      ...
      createLossClaimValidationErrorTest("AA123456A",
        generateLossClaim(Some(selfEmploymentId), typeOfLoss, "2019-25", "carry-forward"), Status.BAD_REQUEST, RuleTaxYearRangeInvalid.copy(paths = Some(List("/taxYear"))))
      createLossClaimValidationErrorTest("AA123456A",
