  ...
  "Calling the create BFLoss endpoint" should {
    ...
    "return 400 (Bad Request)" when {
      ...
      createBFLossValidationErrorTest("AA123456A",
        generateBFLoss(Some(selfEmploymentId), typeOfLoss, "20111", lossAmount) , Status.BAD_REQUEST, TaxYearFormatError.copy(paths = Some(List("/taxYear"))))
      ...
      createBFLossValidationErrorTest("AA123456A",
        generateBFLoss(Some(selfEmploymentId), typeOfLoss, "2019-25", lossAmount), Status.BAD_REQUEST, RuleTaxYearRangeInvalid.copy(paths = Some(List("/taxYear"))))
      createBFLossValidationErrorTest("AA123456A",
