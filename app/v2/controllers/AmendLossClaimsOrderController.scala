...
class AmendLossClaimsOrderController @Inject()(val authService: EnrolmentsAuthService,
    ...
    authorisedAction(nino).async(parse.json) { implicit request =>
      ...
      val result =
        for {
          ...
          auditSubmission(
            AmendLossClaimsOrderAuditDetail(request.userDetails,
                                            nino,
                                            taxYear,
                                            request.body,
  ...
  private def auditSubmission(details: AmendLossClaimsOrderAuditDetail)(implicit hc: HeaderCarrier, ec: ExecutionContext) = {
