...
@Singleton
class AmendLossClaimTypeConnector @Inject() (val http: HttpClientV2, val appConfig: AppConfig) extends BaseDownstreamConnector {
  ...
  def amendLossClaimType(request: AmendLossClaimTypeRequestData)(implicit
    ...
    val downstreamUri: DownstreamUri[DownstreamResp] = HipUri(
      s"itsd/income-sources/claims-for-relief/$nino/$claimId?taxYear=${taxYearClaimedFor.asTysDownstream}")
