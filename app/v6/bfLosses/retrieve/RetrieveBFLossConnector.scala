...
@Singleton
class RetrieveBFLossConnector @Inject() (val http: HttpClientV2, val appConfig: AppConfig) extends BaseDownstreamConnector {
  ...
  def retrieveBFLoss(request: RetrieveBFLossRequestData)(implicit
    ...
    val downstreamUri: DownstreamUri[DownstreamResp] = HipUri(s"itsd/income-sources/brought-forward-losses/$nino/$lossId")
