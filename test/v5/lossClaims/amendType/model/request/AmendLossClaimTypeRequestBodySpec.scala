...
import v5.lossClaims.validators.models.TypeOfClaim._
    ...
    "produce the correct model from MTD submission json" when {
      testData.foreach(test =>
        s"supplied with a TypeOfClaim of ${test._1}" in {
          mtdJson(test._1).as[Def1_AmendLossClaimTypeRequestBody] shouldBe Def1_AmendLossClaimTypeRequestBody(test._2)
        })
    }
  }
