  ...
  "running a validation" should {
    "return no errors" when {
      "a valid request is supplied" in {
        validator.validate(request.AmendLossClaimsOrderRawData(validNino, validTaxYear, AnyContentAsJson(mtdRequest))) shouldBe Nil
      }
    }

    "return NinoFormatError error" when {
      "nino validation fails" in {
        validator.validate(request.AmendLossClaimsOrderRawData("badNino", validTaxYear, AnyContentAsJson(mtdRequest))) shouldBe
    ...
    "return RuleTaxYearRangeInvalid" when {
      "tax year gap is higher than 1" in {
        validator.validate(request.AmendLossClaimsOrderRawData(validNino, "2020-22", AnyContentAsJson(mtdRequest))) shouldBe
    ...
    "return TaxYearFormatError" when {
      "tax year format is invalid" in {
        validator.validate(request.AmendLossClaimsOrderRawData(validNino, "badTaxYear", AnyContentAsJson(mtdRequest))) shouldBe
    ...
    "return RuleTaxYearNotSupportedError" when {
      "tax year is too early" in {
        validator.validate(request.AmendLossClaimsOrderRawData(validNino, "2018-19", AnyContentAsJson(mtdRequest))) shouldBe
    ...
    "return TypeOfClaimFormatError" when {
      "a non-carry-sideways typeOfClaim is provided" in {
        ...
        validator.validate(request.AmendLossClaimsOrderRawData(validNino, validTaxYear, AnyContentAsJson(requestBody))) shouldBe
    ...
    "return RuleIncorrectOrEmptyBodyError" when {
      "empty json is provided" in {
        validator.validate(request.AmendLossClaimsOrderRawData(validNino, validTaxYear, AnyContentAsJson(Json.obj()))) shouldBe
          List(RuleIncorrectOrEmptyBodyError)
      }

      "the typeOfClaim field isn't provided" in {
        validator.validate(request.AmendLossClaimsOrderRawData(validNino, validTaxYear, AnyContentAsJson(mtdRequest.removeProperty("typeOfClaim")))) shouldBe
    ...
    "return multiple errors" when {
      "invalid nino and tax year are provided" in {
        validator.validate(request.AmendLossClaimsOrderRawData("invalid", "13900", AnyContentAsJson(mtdRequest))) shouldBe
