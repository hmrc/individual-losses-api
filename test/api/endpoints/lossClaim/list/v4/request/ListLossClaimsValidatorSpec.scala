...
class ListLossClaimsValidatorSpec extends UnitSpec {
  ...
  def rawData(nino: String = validNino,
              taxYearClaimedFor: String = validTaxYear,
  ...
  "running validation" should {
    "return no errors" when {
      ...
      "a valid request with no parameters is supplied" in {
        validator.validate(rawData(typeOfLoss = None, businessId = None, typeOfClaim = None)) shouldBe Nil
      }
    }
    ...
    "return TaxYearFormatError" when {
      "the tax year is the incorrect format" in {
        validator.validate(rawData(taxYearClaimedFor = "XXXX")) shouldBe List(TaxYearFormatError)
      }
    }

    "return RuleTaxYearRangeInvalid" when {
      "the tax year range is not a single year" in {
        validator.validate(rawData(taxYearClaimedFor = "2018-20")) shouldBe List(RuleTaxYearRangeInvalidError)
      }
    }

    "return RuleTaxYearNotSupportedError" when {
      "the tax year is too early" in {
        validator.validate(rawData(taxYearClaimedFor = "2018-19")) shouldBe List(RuleTaxYearNotSupportedError)
      }
    }
    ...
    "return multiple errors" when {
      "there are multiple errors" in {
        validator.validate(rawData(nino = "badNino", taxYearClaimedFor = "badTaxYear")) shouldBe List(NinoFormatError, TaxYearFormatError)
      }
    }
  }
