...
import api.endpoints.bfLoss.list.v4.request.{ ListBFLossesRawData, ListBFLossesRequest, MockListBFLossesParser }
import api.endpoints.bfLoss.list.v4.response.{ ListBFLossHateoasData, ListBFLossesItem, ListBFLossesResponse }
...
class ListBFLossesControllerSpec
  ...
  private val rawData        = ListBFLossesRawData(nino, taxYear, Some(selfEmployment), Some(businessId))

  private val request =
    ListBFLossesRequest(Nino(nino), TaxYear("2019"), Some(IncomeSourceType.`02`), Some(businessId))
