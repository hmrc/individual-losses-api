...
class ListBFLossesServiceSpec extends ServiceSpec {
  ...
  private def request(taxYear: Option[TaxYear] = Some(TaxYear.fromMtd("2020-21"))): ListBFLossesRequest =
    v3.request.ListBFLossesRequest(Nino(nino), taxYear, None, None)
  ...
  "retrieve the list of bf losses" should {
    ...
    "map INVALID_TAX_YEAR to InternalError if no taxYear is provided" in new Test {
      ...
      await(service.listBFLosses(request(None))) shouldBe Left(ErrorWrapper(correlationId, InternalError))
    }
