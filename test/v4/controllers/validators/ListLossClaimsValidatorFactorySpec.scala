  ...
  "running validation" should {
    "return the parsed request data" when {
      "given a valid request" in {
        ...
        result shouldBe Right(
          ListLossClaimsRequestData(parsedNino, parsedTaxYear, Some(parsedTypeOfLoss), Some(parsedBusinessId), Some(parsedTypeOfClaim))
        )
      }

      "given a valid request with no parameters" in {
        val result = validator(validNino, validTaxYear, None, None, None).validateAndWrapResult()
        result shouldBe Right(
          ListLossClaimsRequestData(parsedNino, parsedTaxYear, None, None, None)
        )
      }
    }
    ...
    "return NinoFormatError" when {
      "given an invalid nino" in {
        val result =
          validator(invalidNino, validTaxYear, Some(validLossType), Some(validBusinessId), Some(validTypeOfClaim)).validateAndWrapResult()
    ...
    "return RuleTaxYearRangeInvalid" when {
      "given a tax year range which isn't a single year" in {
        val result = validator(validNino, "2018-20", Some(validLossType), Some(validBusinessId), Some(validTypeOfClaim)).validateAndWrapResult()
        result shouldBe Left(
    ...
    "return TypeOfLossFormatError" when {
      "given an invalid loss type" in {
        val result =
          validator(validNino, validTaxYear, Some(invalidLossType), Some(validBusinessId), Some(validTypeOfClaim)).validateAndWrapResult()
        result shouldBe Left(
      ...
      "the loss type is not permitted for claims" in {
        val result = validator(validNino, validTaxYear, Some("self-employment-class4"), Some(validBusinessId), Some(validTypeOfClaim))
    ...
    "return TypeOfClaimFormatError" when {
      "given an invalid claim type" in {
        val result =
          validator(validNino, validTaxYear, Some(validLossType), Some(validBusinessId), Some(invalidTypeOfClaim)).validateAndWrapResult()
        result shouldBe Left(
      ...
      "given a claim type that isn't carry-sideways" in {
        TypeOfClaim.values
          ...
          .foreach { typeOfClaim =>
            val result =
              validator(validNino, validTaxYear, Some(validLossType), Some(validBusinessId), Some(typeOfClaim.toString)).validateAndWrapResult()
            result shouldBe Left(
              ErrorWrapper(correlationId, TypeOfClaimFormatError)
            )
          }
    ...
    "return BusinessIdFormatError" when {
      "given an invalid self employment id" in {
        val result =
          validator(validNino, validTaxYear, Some(validLossType), Some(invalidBusinessId), Some(validTypeOfClaim)).validateAndWrapResult()
        result shouldBe Left(
