...
class AmendLossClaimsOrderValidatorFactorySpec extends UnitSpec with JsonErrorValidators {
  ...
  private def validator(nino: String, taxYearClaimedFor: String, body: JsValue) = validatorFactory.validator(nino, taxYearClaimedFor, body)
    ...
    "return the parsed domain object" when {
      "given a valid request" in {
        ...
        result shouldBe Right(
          AmendLossClaimsOrderRequestData(
            parsedNino,
            parsedTaxYear,
            AmendLossClaimsOrderRequestBody(TypeOfClaim.`carry-sideways`, List(Claim("AAZZ1234567890a", 1))))
        )
      }
    }
