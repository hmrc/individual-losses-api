...
class DeleteLossClaimValidatorFactorySpec extends UnitSpec {
  ...
  private def validator(nino: String, claimId: String): Validator[DeleteLossClaimRequestData] = validatorFactory.validator(nino, claimId)
    ...
    "return the parsed request data" when {
      "given a valid request" in {
        ...
        result shouldBe Right(
          DeleteLossClaimRequestData(parsedNino, parsedClaimId)
        )
      }
    }
