...
import v3.connectors.{IfsOutcome, LossClaimConnector}
...
trait MockLossClaimConnector extends MockFactory {
  ...
  object MockedLossClaimConnector {
    ...
    def createLossClaim(request: CreateLossClaimRequest): CallHandler[Future[IfsOutcome[CreateLossClaimResponse]]] = {
      (connector
        .createLossClaim(_: CreateLossClaimRequest)(_: HeaderCarrier, _: ExecutionContext))
        .expects(request, *, *)
    }

    def amendLossClaim(amendLossClaimRequest: AmendLossClaimRequest): CallHandler[Future[IfsOutcome[LossClaimResponse]]] = {
      (connector
        .amendLossClaim(_: AmendLossClaimRequest)(_: HeaderCarrier, _: ExecutionContext))
        .expects(amendLossClaimRequest, *, *)
    }

    def retrieveLossClaim(request: RetrieveLossClaimRequest): CallHandler[Future[IfsOutcome[LossClaimResponse]]] = {
      (connector
        .retrieveLossClaim(_: RetrieveLossClaimRequest)(_: HeaderCarrier, _: ExecutionContext))
        .expects(request, *, *)
    }

    def deleteLossClaim(deleteLossClaimRequest: DeleteLossClaimRequest): CallHandler[Future[IfsOutcome[Unit]]] = {
      (connector
        .deleteLossClaim(_: DeleteLossClaimRequest)(_: HeaderCarrier, _: ExecutionContext))
        .expects(deleteLossClaimRequest, *, *)
    }

    def listLossClaims(request: ListLossClaimsRequest): CallHandler[Future[IfsOutcome[ListLossClaimsResponse[LossClaimId]]]] = {
      (connector
        .listLossClaims(_: ListLossClaimsRequest)(_: HeaderCarrier, _: ExecutionContext))
        .expects(request, *, *)
    }

    def amendLossClaimsOrder(request: AmendLossClaimsOrderRequest): CallHandler[Future[IfsOutcome[Unit]]] = {
      (connector
