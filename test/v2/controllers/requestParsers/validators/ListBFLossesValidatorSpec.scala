  ...
  "running validation" should {
    "return no errors" when {
      ...
      "a business id is not supplied for a loss type of self-employment" in {
        validator.validate(
          ListBFLossesRawData(nino = validNino, taxYear = Some(validTaxYear), typeOfLoss = Some("self-employment"), businessId = None)) shouldBe Nil
      }

      "a business id is supplied and no loss type is supplied" in {
        validator.validate(ListBFLossesRawData(nino = validNino, taxYear = Some(validTaxYear), typeOfLoss = None, businessId = Some(validBusinessId))) shouldBe Nil
      }
    }
    ...
    "not return an error" when {
      ...
      "a business id is not supplied for a loss type of fhl property" in {
        validator.validate(
          ListBFLossesRawData(nino = validNino, taxYear = Some(validTaxYear), typeOfLoss = Some("uk-property-fhl"), businessId = None)) shouldBe Nil
      }
